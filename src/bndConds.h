...
public:
  ...
  REAL machinf,  /**< Mach-number at infinity (far-field) */
      ...
      cl;        /**< lift coefficient (pressure forces only; used for vortex
                    correction) */
  ...
  REAL ptinl,  /**< total pressure at inlet */
      ...
      betainl, /**< low angle at inlet (with x-axis, positive in the
                  clock-wise direction) */
      betaout, /**< approximate outlet angle (utilized for the initial guess
                  only) */
      p12rat,  /**< ratio of inlet to outlet static pressure (initial guess
                * only)
                */
  ...
  static void ZeroResiduals(const Geometry &geometry, Equations equsType,
                            CONSVARS rhs[]);

private:
  PRIMVARS *bndVals; /**< boundary values of density, u, v, and p (used for
                        far-field BC) */
  ...
  void BcondInflow(const Geometry &geometry, FluidProps &fluidProps, int ibegn,
                   int iendn) const;
  void BcondOutflow(const Geometry &geometry, FluidProps &fluidProps, int ibegn,
                    int iendn) const;
  void BcondFarfield(const Geometry &geometry, FluidProps &fluidProps,
                     const Precond &precond, int ibegn, int iendn);
  void BcondWallVisc(const Geometry &geometry, FluidProps &fluidProps,
